...
func TestAnnotateContext_WorksWithEmpty(t *testing.T) {
	...
	if err != nil {
		t.Errorf("AnnotateContext(ctx, %#v) failed with %v; want success", request, err)
		return
	}
...
func TestAnnotateContext_ForwardsGrpcMetadata(t *testing.T) {
	...
	if m, ok := RPCMethod(annotated); !ok {
		t.Errorf("RPCMethod(annotated) failed with no value; want %s", expectedRPCName)
	} else if m != expectedRPCName {
		t.Errorf("RPCMethod(annotated) failed with %s; want %s", m, expectedRPCName)
	}

	if m, ok := HTTPPathPattern(annotated); !ok {
		t.Errorf("HTTPPathPattern(annotated) failed with no value; want %s", expectedHTTPPathPattern)
	} else if m != expectedHTTPPathPattern {
		t.Errorf("HTTPPathPattern(annotated) failed with %s; want %s", m, expectedHTTPPathPattern)
	}
}
...
func TestAnnotateIncomingContext_WorksWithEmpty(t *testing.T) {
	...
	if err != nil {
		t.Errorf("AnnotateIncomingContext(ctx, %#v) failed with %v; want success", request, err)
		return
	}
