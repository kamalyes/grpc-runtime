		...
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			for _, p := range spec.patterns {
				func(p stubPattern) {
					...
					if err != nil {
						t.Fatalf("NewPattern(1, %#v, %#v, %q) failed with %v; want success", p.ops, p.pool, p.verb, err)
					}
...
func TestServeMux_StaticPathFastPath(t *testing.T) {
	...
	if _, ok := mux.staticHandlers[http.MethodGet]["/v1/health"]; !ok {
		t.Fatal("static handler was not indexed")
	}
...
func TestServeMux_DynamicPathNotStaticIndexed(t *testing.T) {
	...
	if handlers := mux.staticHandlers[http.MethodGet]; len(handlers) != 0 {
		t.Fatalf("static handlers = %v; want none", handlers)
	}

	w := httptest.NewRecorder()
	mux.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/v1/alice", nil))

	if got, want := w.Body.String(), "alice"; got != want {
		t.Fatalf("body = %q; want %q", got, want)
	}
}
